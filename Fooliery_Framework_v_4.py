...
tv = [pygame.image.load('Images/tv1.jpg'),
      pygame.image.load('Images/tv2.jpg'),
      pygame.image.load('Images/tv3.jpg'),
      pygame.image.load('Images/tv4.jpg'),
      pygame.image.load('Images/tv5.jpg'),
      pygame.image.load('Images/tv6.jpg'),
      pygame.image.load('Images/tv7.jpg'),
      pygame.image.load('Images/tv8.jpg')]
#images path go here
...
class tester(pygame.sprite.Sprite):
    def __init__(self,game):
        ...
        self.course = random.choice([trajectory_rect(a,b,c,f,h),trajectory_octagon(a,b,c,cc,h),trajectory_triangle(a,b,c,cc,h),
...
def display_timed_string(g, list, support, paste_in='midtop'):

    screen_text_flex(support[2], g.tsupport, 300, 300, paste_in=paste_in)
...
def text_window_basic(text,support,x,y,text_window_setup,display_time=5,type='basic',tag_display=''):
    ...
    if tag_display != 'fixed':
        ...
        if text_window_setup[0]:
            ...
            if type == 'flash':
                screen_text_flashing(text,e,text_window_setup[2].width // 2, 10, 'midtop')
        if text_window_setup[0]:
...
def text_window_stack(text1,text2,text3,support,x,y,text_window_setup1,text_window_setup2,text_window_setup3,display_time=5,type='basic'):
    a = 50
    text_window_basic(text1,support,x,y,text_window_setup1,display_time,type,tag_display='fixed')#text,support,x,y,text_window_setup,display_time=5
...
def text_window_page(text_list,support,x,starty,wlist):
    ...
    for i in range(len(text_list)):
        text_window_basic(text_list[counter], support, x, starting_y,wlist[counter2], display_time=10000000,tag_display='fixed')
        counter += 1
         ...
         for row in self.xlist:#this is the x of the q
             for col in self.ylist:#this is the y of the q
                 if row +self.tile_size > self.game.mpos[0] > row and col + self.tile_size > self.game.mpos[1] > col:
                    ...
                    self.xpos = self.r - 2
                    self.ypos = self.c - 2
         if self.x_space + (self.tile_size * 14) > self.game.mpos[0] > self.x_space and self.y_space + (self.tile_size * 4) > self.game.mpos[1] > self.y_space:
...
class Wall(pygame.sprite.Sprite):
    ...
    def Room(self):
        ...
        pass
class build_wall:
    def __init__(self,game, x, y,x_to,y_to,length,tile_size = TILESIZE,img = None,color = black,does_collide = False,collider_group = None,do_kill = False):
        self.game = game
        endx = 0
        endy = 0
        self.img = img
...
def black_fill_back():
    ...
    return a
#######################################################################################################################
def easybutton_support(g,x,y,sizex,sizey,text):
    ...
    h = a_rect.height
    flux = bool_loop(0,10)
    screen_text_flex(text, g.tsupport,3, 3, paste_in='topleft',surface_tag=a)
    sx = a_rect.topleft[0]
    sy = a_rect.topleft[1]
...
######################################################################################################################
def key_input(g,question_txt):
    running = True

    while running:
        g.screen.fill(black)
        screen_text_flex(question_txt,g.tsupport,300,200)
...
########################################################################
def txt_input(g,question_txt,save_to_file):
    running = True
    ...
    while running:
        ...
        screen_text_flex(newstring,g.tsupport,300,300)

        pygame.display.flip()

def Y_N_O_box(g,question,yes_option,no_option):
    running = True
    supportclick = easybutton_support(g, 350, 400, 150, 50,yes_option)
    supportclick1 = easybutton_support(g, 355, 450, 150, 50, no_option)
    clicked = False
    while running:
        g.screen.fill(black)
        for e in pygame.event.get():
        ...
        if button_todo(supportclick):

            running = False
            return True
        if button_todo(supportclick1):

            running = False
            return False

        screen_text_flex(question,g.tsupport_small,300,100)
        pygame.display.flip()
...
def textPageloop(g,textlist,kill_time_in_secs):
    running = True
    ...
    while running:
        ...
        for i in range(len(textlist1)):
            screen_text_flex(textlist1[counter],g.tsupport_small,wth//2,y)
...
#####################################################################################################33
def txt_input_with_return(g,question_txt):
    running = True
    runone = False
    Name = []
    startx = 300
    ...
    while running:
        ...
        if len(stringthis) == 160:
            ...
            startx -= 130
        newstring = stringthis
        screen_text_flex(newstring,g.tsupport,startx,300)
        runone = True
        pygame.display.flip()
